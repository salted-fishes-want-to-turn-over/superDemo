...
public class DepartmentService {
    ...
    public ResponseDTO<List<DepartmentVO>> listAllDepartmentEmployee(String departmentName) {
        ...
        employeeList.forEach(employeeDTO -> { // 依次遍历员工表，逐一将员工添加到各自的部门

            DepartmentVO departmentVO = departmentMap.get(employeeDTO.getDepartmentId());// 通过员工获取部门
            if (null == departmentVO) {
                return;
            }
            List<EmployeeDTO> employeeDTOList = departmentVO.getEmployees(); //查出该部门下未添加这个员工前拥有的员工
            if (null == employeeDTOList) {
                employeeDTOList = new ArrayList<>();
            }
            employeeDTOList.add(employeeDTO);  // 添加新的员工到部门
    ...
    private List<DepartmentVO> filterDepartment(List<DepartmentVO> departmentVOList, String departmentName) {
        ...
        departmentVOList.forEach(item -> {
            if (item.getName().indexOf(departmentName) < 0) { //对于部门名称不为该departmentName的节点进行晒除
                return;
            }
            // 当前部门包含关键字
            ...
            if (null != parentId) {
                ...
                getParentDepartment(departmentVOList, parentId, filterResult); //通过parentId在列表中找到当前节点的父节点，祖父节点，，，直至根节点
