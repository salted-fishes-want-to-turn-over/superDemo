package cn.yubajin.smartadmin.util;

import cn.yubajin.smartadmin.common.domain.OrderItemDTO;
...
public class SmartPageUtil {
    ...
    public static <T> Page<T> convert2QueryPage(PageParamDTO baseDTO) {
        ...
        if (orders != null && !orders.isEmpty()) {
            List<OrderItem> orderItemList = orders.stream().map(SmartPageUtil::convertOrderItem).collect(Collectors.toList());
